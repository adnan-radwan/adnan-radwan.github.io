...
(function() {
  ...
  window.addEventListener('scroll', () => {
    ...
    if (rect.top < windowHeight && rect.bottom > 0) {
      ...
      heroVisual.style.transform = `translateY(${-12 * progress}px)`;
    }
  });
})();
...
(function() {
  ...
  toggleBtn.addEventListener('click', () => {
    const isGold = document.body.classList.toggle(BODY_CLASS);
    localStorage.setItem('ga-theme', isGold ? 'gold' : 'dark');
  });
})();
</script>
...
<script>
/* ============================================================
   GOLDEN ADS — Data Engine (adsDB.js)
============================================================ */

/* 6) Categories Bar */
function loadCategoriesBar() {
  const bar = document.getElementById('gaCatsBar');
  if (!bar || !window.categoriesDB) return;

  bar.innerHTML = categoriesDB.map(cat => `
    <div class="ga-cat-pill cine-fade">
      <span class="icon">${cat.icon}</span>
      <span class="text">${cat.name_ar}</span>
    </div>
  `).join('');
...
function loadFeatured() {
  ...
  const featured = adsDB
    .filter(ad => ad.status === "active" && ad.featured)
    .sort((a, b) => b.id - a.id)
    .slice(0, 20);
...
function loadDecor(loadMore = false) {
  ...
  const decor = adsDB
    .filter(ad => ad.category === "decor" && ad.status === "active")
    .sort((a, b) => b.id - a.id);

  const slice = decor.slice(
    decorPage * decorLimit,
    (decorPage + 1) * decorLimit
  );
...
function loadLatest() {
  ...
  label.textContent = `${latest.length} إعلان`;
}
...
document.addEventListener('input', () => {
  const q = document.getElementById('gaSearchInput');
  if (!q) return;
  ...
  label.textContent = `${list.length} إعلان`;
});
...
<script src="https://raw.githubusercontent.com/adnan-radwan/adnan-radwan.github.io/30b6bb21e86e29e80cd30c1240ba125cb0fc2516/ad-view/adsDB.js">
